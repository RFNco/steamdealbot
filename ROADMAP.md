...
**Latest tagged release:** v2.1.8

Future ideas and improvements to consider after v2.1.8. (See **CHANGELOG [Unreleased]** for work in progress toward the next tag.)

## Content Quality

- [x] Add deal modes: big names only, popular indies, hidden gems, deep discounts, under $10, 50%+ off favorites.
- [x] Deal modes / Categories sample randomized Steam pages for more variety on each open (same idea as Nintendo shuffle).
...
- [x] Add category searches such as RPG, horror, co-op, cozy, strategy, action, open world, and under $5.
...
- [x] Add more reusable templates for Steam, Nintendo, and general gaming posts (`TWEET_IDEA_THEMES` / `TWEET_IDEA_THEME_EXTRAS` in `manual_poster.py`). **Tweet ideas last rolled: v2.1.8**
...
- [x] Main-menu **Gaming news** (replaces bulk Copy 5 deals) for faster RSS access.
- [ ] Add a daily posting mix suggestion, for example 2 popular games, 2 indie games, 1 news headline, and 1 engagement tweet.
...
- [x] Detect Cloudflare-blocked news image CDNs (e.g. Nintendo Life) and fall back to copy URL / open in browser.
...
- [x] Main-menu Gaming news (10/page, **0** = next batch / re-fetch at end of pool).
...
- [x] Add dedupe + cache for news items to avoid reposting the same headline too often (posted-news memory, similar to deals).
- [x] Owned blog feed (**Stathetic Blog**) with freshness window (hide posts older than N days) and light pin-to-top.
- [ ] Configurable / expandable feed list via config file (add/remove outlets without code edits).
- [ ] Balance feed mix so one high-volume outlet does not dominate the pool.
- [ ] Add source filters (official-only, platform-specific, owned-blog-only, all sources).
- [ ] Add recency filters (last 6h, 24h, 3d) for outlet headlines (owned blog already uses a 2-day window).
- [ ] Optional UTM (or similar) params on owned-blog links for visitor analytics.
- [x] Per-release habit: refresh a few `QUESTION_OPENERS` / `HYPE_OPENERS` lines when tagging (same idea as tweet-idea rolls).
- [ ] Buffer for news: save uncertain items as Buffer Ideas so they don’t burn limited queue slots (`addToQueue` already offered after copy).
